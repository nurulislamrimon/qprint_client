...
import ModalCloseBtn from "@/components/shared/ModalCloseBtn";
import { usePrintingRequestOrderCancelMutation } from "@/redux/features/printing-request/printing-request";

const PrintingRequestOrderCancelledModal = ({
  showModal,
  setShowModal,
  orderId,
}: any) => {
  ...
  const [cancelOrder] = usePrintingRequestOrderCancelMutation(orderId);
  ...
  const handleOrderCancel = async (e: any) => {
    ...
    try {
      ...
      if ("data" in res) {
        ...
        router.push(`/profile/printing-order-history`);
      }
    } catch (err: any) {
      ...
      >
        <div className=" bg-white p-7 rounded-lg">
          ...
          <h4 className="text-black text-opacity-80 text-[18px] font-semibold mb-7 md:mb-11 text-center">
            Printing Request Cancellation Request
          </h4>
...
export default PrintingRequestOrderCancelledModal;
